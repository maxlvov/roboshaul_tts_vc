...
class AcousticModel(nn.Module):
    ...
    def generate(self, x: torch.Tensor, mel0 = []) -> torch.Tensor:
        x = self.encoder(x)
        return self.decoder.generate(x, mel0)
...
class Decoder(nn.Module):
    ...
    @torch.inference_mode()
    def generate(self, xs: torch.Tensor, mel0 = []) -> torch.Tensor:
        ...
        mel = []
        for i,x in enumerate(torch.unbind(xs, dim=1)):
            m = self.prenet(m)
