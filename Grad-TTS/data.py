...
#Preload all encodec files into memory to speed up
encodec_files = glob.glob("/content/encodec_files/*.npy")
dic_fil2emb={}
for fil in tqdm.tqdm(encodec_files) :
    utt_emb = np.load(fil).squeeze()
    dic_fil2emb[fil] = torch.Tensor(utt_emb)
...
class TextMelDataset(torch.utils.data.Dataset):
    ...
    def get_mel(self, filepath):
        ...
        if sr!=self.sample_rate :
            audio = torch.Tensor(signal.resample(audio.numpy(), int(len(audio) * float(self.sample_rate) / sr)))
            sr = self.sample_rate
    ...
    '''
    def get_mel(self, filepath):
        utt = os.path.basename(filepath)[:-4]
        fil = "/content/encodec_files/"+utt+".npy"
        return dic_fil2emb[fil]
