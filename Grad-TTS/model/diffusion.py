...
class LinearAttention(BaseModule):
    ...
    def forward(self, x):
        ...
        b,baa,h,w = qkv.size()
        c=32
        if baa!=3*self.heads*c :
            print(qkv.size())
            print(self.heads)
            print(x.shape)
        assert(baa==3*self.heads*c)
        qkv = qkv.permute(0,2,3,1) #[b,h,w,(3 heads c)]
        qkv = qkv.reshape(b,h,w,3,self.heads,c) #[b,h,w,3,heads,c]
        qkv = qkv.permute(3,0,4,5,1,2) #[3,b,heads,c,h,w]
        qkv = qkv.reshape(3,b,self.heads,c,h*w)
        q, k, v = qkv
        ...
        b,heads,c,baa = out.size()
        assert(baa==h*w and heads==self.heads)
        out = out.reshape(b,heads,c,h,w)
        out = out.permute(0,3,4,1,2) #[b,h,w,heads,c]
        out = out.reshape(b,h,w,heads*c) #[b,h,w,heads*c]
        out = out.permute(0,3,1,2) #[b,heads*c,h,w]
